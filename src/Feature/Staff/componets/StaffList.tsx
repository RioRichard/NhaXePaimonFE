...
import { Staff, StaffListProps } from '../types';
import Tippy from '@tippyjs/react';
import CreateIcon from '@mui/icons-material/Create';
import DeleteIcon from '@mui/icons-material/Delete';
import 'tippy.js/dist/tippy.css'
//Giao diện trang chủ hiển thị tất cả Account mà Admin quản lý
export default function BaseList(props: StaffListProps) {
    const { rows, onStaffDeleteClick, onStaffEditClick } = props;
    
    const handleEditClick = (row: Staff) => {
        if (onStaffEditClick) onStaffEditClick?.(row);
    };
    const handleDeleteClick = (row: Staff) => {
        if (onStaffDeleteClick) onStaffDeleteClick?.(row);
    };
    return (
